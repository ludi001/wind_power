...
let Comps = React.createClass({
    ...
    render () {
        ...
        let {size,data,maxData,minData,datasourse,left,top,title,height,width,unit,color}=this.state;
        ...
        return (
            <div className={styles.mainBox} id='allbox'>
                ...
                <label>当前值:</label><input type='number'  defaultValue={data} onChange={this.handleChange5} data-theme={'data'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>
                {/*<label>最大值:</label><input type='number'  defaultValue={maxData} onChange={this.handleChange6} data-theme={'maxData'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>*/}
                {/*<label>最小值:</label><input type='number'  defaultValue={minData} onChange={this.handleChange7} data-theme={'minData'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>*/}
                <label>颜色:</label><input id='box' defaultValue={color} onChange={this.handleChange4} className={styles.colorselect}  data-theme={'color'}  onBlur={()=> setCompsDetail(compsList, activeComps)}/>
            </div>
        )
    }
})
