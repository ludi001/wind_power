...
let Comps = React.createClass({
   getInitialState(){
    ...
    console.log('getInitialState',newobj);
     return{
       ...
       allheight:newobj.detail!=undefined?newobj.detail.allheight:100,
       ...
       titlebg:newobj.detail!=undefined?newobj.detail.titlebg:'#ccc',
       allbg:newobj.detail!=undefined?newobj.detail.allbg:'#fff',
 ...
 handleChange3(e){
   this.setState({allheight: e.target.value});
 },
  handleChange4(e){
 ...
 handleChange7(e){
  ...
  handleChange12(e){
  this.setState({titlebg: e.target.value});
 },
  handleChange13(e){
  this.setState({allbg: e.target.value});
 },
  render () {
    const {activeComps, compsList, setCompsDetail} = this.props;
    console.log(22,this.props)
    let {tit,titleheight,allwidth,allheight,marginLeft,marginTop,fontSize,color,borderWidth,borderStyle,borderColor,textalign,titlebg,allbg}=this.state;
    ...
    return (
      <div className={styles.mainBox} id='allbox'>
        ...
        <label>标题高度px:</label><input type='number' min={0} defaultValue={titleheight} onChange={this.handleChange1} data-theme={'titleheight'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>
        <label>整体宽度%:</label><input type='number' min={0} defaultValue={allwidth} onChange={this.handleChange2} data-theme={'width'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>
        <label>整体高度%:</label><input type='number' min={0} defaultValue={allheight} onChange={this.handleChange3} data-theme={'allheight'}   onChange={()=> setCompsDetail(compsList,activeComps)}/>
        ...
        <label>标题背景:</label><input id='backbox' className={styles.colorselect} defaultValue={titlebg} onChange={this.handleChange12}     data-theme={'titlebg'}  onBlur={()=> setCompsDetail(compsList, activeComps)}  />
        <label>整体背景:</label><input id='backbox1' className={styles.colorselect} defaultValue={allbg} onChange={this.handleChange13}     data-theme={'allbg'}  onBlur={()=> setCompsDetail(compsList, activeComps)}  />
